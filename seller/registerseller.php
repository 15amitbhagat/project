...
<?php 
    ...
    if(isset($_POST['submit'])){
        ...
        $select = mysqli_query($conn,"SELECT * from seller where email='$email' ");
        if(mysqli_num_rows($select)==0){
          $hash =  md5( $password) ;
        $insert =  mysqli_query($conn,"INSERT INTO seller(name,email,password) VALUES('$name','$email','$hash')");
          if($insert){
            $success = 'Successfully Register!';
            header("refresh:5;location: logins.php");
          }
          else{
...
<body>
  ...
  <div class="container mt-4">
    <div class="card">
      ...
      <div class="card-body">
        <!-- register for seller will be registers -->
         <form action="<?php echo $_SERVER['PHP_SELF']?>"  method="POST">
               ...
               <div class="alert alert-success" role="alert">
                 <?php echo $success;?><br>
                 <a href="project/seller/loginseller.php">Go to login Page!</a>
               </div>
               <?php }?>
      </div>
    </div>
